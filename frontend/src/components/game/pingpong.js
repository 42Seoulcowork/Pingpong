...
function paddle(position_x, position_z, color) {
  const geometry = new THREE.BoxGeometry(1.5, 1.5, 5);
...
if (WebGL.isWebGLAvailable()) {
  ...
  const paddle1 = paddle(-13, 0, 0x00ff00);
  ...
  document.addEventListener("keydown", function (event) {
    ...
    if (keysPressed["w"]) paddle1.position.z -= 1;
    if (keysPressed["s"]) paddle1.position.z += 1;
    if (keysPressed["ArrowUp"]) paddle2.position.z -= 1;
    if (keysPressed["ArrowDown"]) paddle2.position.z += 1;
  });
